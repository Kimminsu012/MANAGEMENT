...
public class DesignerController {
    ...
    public String salaryList(@RequestParam(name = "page", required = false, defaultValue = "0") Integer page, Model model){
        ...
        int maxPage = (int)Math.ceil((double) designerDtoList.size()/10);
        model.addAttribute("designerDtoList", designerDtoList.subList(page * 10, Math.min((page + 1) * 10, designerDtoList.size())));
        model.addAttribute("maxPage",maxPage);
        model.addAttribute("currentPage", page);
